...
bool ImGui::Keybind(const char* label, const char* hint_label, int* key)
{
  ...
  //  ImGui::SetCursorPos(GetCursorPos() - ImVec2(0, 10.f));

    const ImRect rect(window->DC.CursorPos, window->DC.CursorPos + ImVec2(width, 28.f));
    ...
    ImRect clickable(ImVec2(rect.Max.x - 30 - label_size.x, rect.Min.y), rect.Max);
    bool hovered = ItemHoverable(clickable, id);

    window->DrawList->AddRectFilled(clickable.Min, clickable.Max, GetColorU32(c::keybind::background), 0.f);
    window->DrawList->AddText(rect.Min - ImVec2(1, 1), GetColorU32(c::keybind::text), label);
    window->DrawList->AddText(rect.Min - ImVec2(1, -14), GetColorU32(c::keybind::hint_text), hint_label);

    PushStyleColor(ImGuiCol_Text, c::keybind::text);
    RenderTextClipped(clickable.Min - ImVec2(0, 0), clickable.Max - ImVec2(0, 0), buf_display, NULL, &label_size, ImVec2(0.5f, 0.5f));
    PopStyleColor();
    ...
    if (hovered_key && g.IO.MouseClicked[1] || it_anim->second.active && (g.IO.MouseClicked[0] || g.IO.MouseClicked[1]) && !it_anim->second.hovered)
    ...
    if (it_anim->second.alpha >= 0.01f)
    {
        ...
        SetNextWindowSize(ImVec2(69 + 20, 90));
...
void ImGui::TextSeparator(ImU32 col, const char* text)
{
    SetCursorPos(GetCursorPos() + ImVec2(0, 6));

    PushFont(fonts::roboto);
...
bool ImGui::Tabs(bool selected, const char* ico, const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, selected ? c::tabs::background_active : c::tabs::background_inactive, g.IO.DeltaTime * 10.f);
    it_anim->second.inflating = ImLerp(it_anim->second.inflating, selected ? ImVec2(0.f, 0.f) : ImVec2(25.f, 25.f), g.IO.DeltaTime * 10.f);
    it_anim->second.text = ImLerp(it_anim->second.text, selected ? c::general_color : hovered ? c::tabs::text_hov : c::tabs::text, g.IO.DeltaTime * 6.f);

    GetWindowDrawList()->AddRectFilled(bb.Min + it_anim->second.inflating, bb.Max - it_anim->second.inflating, GetColorU32(it_anim->second.background), s::tabs::rounding);

    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text));

    PushFont(fonts::icon);
    RenderTextClipped(bb.Min, bb.Max, ico, NULL, &ico_size, ImVec2(0.5f, 0.3f), &bb);
    PopFont();

    PushFont(fonts::roboto);
    RenderTextClipped(bb.Min, bb.Max, label, NULL, &label_size, ImVec2(0.5f, 0.7f), &bb);
    PopFont();
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    if (size.x <= 0.f) size.x = size_arg.x;

    if (size.y <= 0.f) size.y = 30;

    const ImRect rect(pos - ImVec2(0, 0), pos + size - ImVec2(0, 0));

    ItemSize(ImRect(rect.Min, rect.Max - ImVec2(0, 9)));
    if (!ItemAdd(rect, id))
        return false;

    bool hovered, held, pressed = ButtonBehavior(rect, id, &hovered, &held, flags);
    ...
    if (IsItemClicked())
    {
        ...
        it_anim->second.size = 0.f;

        it_anim->second.pos = GetMousePos();
        it_anim->second.active = true;
    }

    it_anim->second.alpha = ImClamp(it_anim->second.alpha + (4.f * g.IO.DeltaTime * (it_anim->second.active ? 1.f : -1.f)), 0.0f, 1.f);

    it_anim->second.size += (size.x / 7.f) * g.IO.DeltaTime * 3.f;

    if (it_anim->second.alpha <= 0.0f)
        it_anim->second.size = 0.f;

    if (it_anim->second.size >= size.x / 6.f)
        it_anim->second.active = false;

    RenderFrame(rect.Min, rect.Max, GetColorU32(c::button::background), false, 0.f);

    PushStyleVar(ImGuiStyleVar_Alpha, 0.5f * it_anim->second.alpha);
    window->DrawList->PushClipRect(rect.Min, rect.Max, true);
    window->DrawList->AddCircleFilled(it_anim->second.pos, it_anim->second.size, GetColorU32(c::general_color), 100.f);
    window->DrawList->PopClipRect();
    PopStyleVar();

    RenderTextClipped(rect.Min + ImVec2(0, 1), rect.Max + ImVec2(0, 1), label, NULL, &label_size, style.ButtonTextAlign, &rect);
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    window->DrawList->AddRectFilled(grab_rect.Min, grab_rect.Max, GetColorU32(c::general_color), 0.f);
...
struct check_state {
    ...
    ImVec4 background, circle, hint, text;
    float circle_pos;
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const float w = GetContentRegionMax().x - style.WindowPadding.x * 2;
    const float square_sz = 20.f;
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + ImVec2(square_sz + (label_size.x > 0.0f ? w : 0.0f), label_size.y));
    ItemSize(total_bb, style.FramePadding.y);
    ...
    it_anim->second.circle = ImLerp(it_anim->second.circle, *v ? c::general_color : c::checkbox::circle_inactive, g.IO.DeltaTime * 6.f);
    it_anim->second.circle_pos = ImLerp(it_anim->second.circle_pos, *v ? 10.f : -10.f, g.IO.DeltaTime * 10.f);

    const ImRect check_bb(pos, pos + ImVec2(square_sz, square_sz));

    GetWindowDrawList()->AddRectFilled(check_bb.Min + ImVec2(w - 20, 0), check_bb.Max + ImVec2(w, 0), GetColorU32(c::checkbox::background), 30.f);
    GetWindowDrawList()->AddCircleFilled(check_bb.Min + ImVec2(w + it_anim->second.circle_pos, 10), 8.f, GetColorU32(it_anim->second.circle), 100.f);

    if (label_size.x > 0.0f) RenderText(ImVec2(check_bb.Max.x - square_sz, check_bb.Min.y), label);

    return pressed;
}

bool ImGui::Checkbox1(const char* label, const char* hint_label, bool* v)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    RenderText(ImVec2(check_bb.Max.x - square_sz, check_bb.Min.y - 4), label);

    PushStyleColor(ImGuiCol_Text, GetColorU32(c::checkbox::hint_text));
    RenderText(ImVec2(check_bb.Max.x - square_sz, check_bb.Min.y + 10), hint_label);
    PopStyleColor();
...
bool ImGui::BeginCombo(const char* label, const char* hint_label, const char* preview_value, ImGuiComboFlags flags)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    ...
    ImGui::SetCursorPos(GetCursorPos() + ImVec2(0, (label_size.y * 2) + 5));

    const ImRect bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, 31.f));
    const ImRect total_bb(bb.Min, bb.Max + ImVec2(label_size.x > 0.0f ? label_size.x : 0.0f, 0.0f));

    ItemSize(ImRect(total_bb.Min, total_bb.Max - ImVec2(0, 10)));
    ...
    it_anim->second.text = ImLerp(it_anim->second.text, popup_open ? c::combo::text : c::combo::hint_text, g.IO.DeltaTime * 6.f);

    window->DrawList->AddRectFilled(bb.Min, bb.Max, GetColorU32(c::combo::background), style.FrameRounding);

    RenderTextClipped(bb.Min + ImVec2(7, 8), bb.Max + ImVec2(0, 8), preview_value, NULL, NULL);

    GetWindowDrawList()->AddText(fonts::icon, 18, ImVec2(bb.Min.x + (w - 27.f), bb.Max.y - 25.f), GetColorU32(it_anim->second.text), "L");

    RenderText(ImVec2(bb.Max.x - w, bb.Min.y - 39.f), label);

    PushStyleColor(ImGuiCol_Text, GetColorU32(c::combo::hint_text));
    RenderText(ImVec2(bb.Max.x - w, bb.Min.y - 25.f), hint_label);
    PopStyleColor();
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
    ...
    PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(g.Style.FramePadding.x, 10.f));
    PushStyleColor(ImGuiCol_PopupBg, GetColorU32(c::combo::background_selectable));
...
bool ImGui::Combo(const char* label, const char* hint_label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int popup_max_height_in_items)
{
    ImGuiContext& g = *GImGui;
    ...
    if (!BeginCombo(label, hint_label, preview_value, ImGuiComboFlags_None))
        return false;
...
bool ImGui::Combo(const char* label, const char* hint_label, int* current_item, const char* const items[], int items_count, int height_in_items)
{
    const bool value_changed = Combo(label, hint_label, current_item, Items_ArrayGetter, (void*)items, items_count, height_in_items);
    return value_changed;
}

// Combo box helper allowing to pass all items in a single string literal holding multiple zero-terminated items "item1\0item2\0"
bool ImGui::Combo(const char* label, const char* hint_label, int* current_item, const char* items_separated_by_zeros, int height_in_items)
{
    int items_count = 0;
    ...
    bool value_changed = Combo(label, hint_label, current_item, Items_SingleStringGetter, (void*)items_separated_by_zeros, items_count, height_in_items);
    return value_changed;
}
...
void ImGui::MultiCombo(const char* label, const char* hint_label, bool variable[], const char* labels[], int count)
{
    ImGuiContext& g = *GImGui;
    ...
    if (ImGui::BeginCombo(label, hint_label, preview.c_str())) // draw start
    {
      ...
      //  SetCursorPos(GetCursorPos() + ImVec2(0, 26));
        PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(g.Style.FramePadding.x, 10));
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    SetCursorPos(GetCursorPos() + ImVec2(0, label_size.y + 5));

    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, 15.f));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));

    const bool temp_input_allowed = (flags & ImGuiSliderFlags_NoInput) == 0;
    ItemSize(ImRect(total_bb.Min, total_bb.Max - ImVec2(0, 16)));
    if (!ItemAdd(total_bb, id, &frame_bb, temp_input_allowed ? ImGuiItemFlags_Inputable : 0))
        return false;
    ...
    GetWindowDrawList()->AddRectFilled(frame_bb.Min + ImVec2(0, 6.5f), frame_bb.Max - ImVec2(0, 6.5f), GetColorU32(c::slider::background));
    ...
    if (grab_bb.Max.x > grab_bb.Min.x) {
        ...
        window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(0, 6), frame_bb.Min + ImVec2(it_anim->second.slow - 3, 8), GetColorU32(c::general_color));

        window->DrawList->AddCircleFilled(ImVec2(frame_bb.Min.x + it_anim->second.slow, grab_bb.Max.y - 6.5f), 8.f, GetColorU32(c::general_color), 30.f);
    }

    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);

    RenderTextClipped(frame_bb.Min - ImVec2(0, 50), frame_bb.Max - ImVec2(0, 0), value_buf, value_buf_end, NULL, ImVec2(1.0f, 0.5f));

    if (label_size.x > 0.0f) RenderText(ImVec2(frame_bb.Max.x - w, frame_bb.Min.y - 25.f), label);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    else
    {
        ...
        ItemSize(ImRect(total_bb.Min, total_bb.Max - ImVec2(0, 9)));
    ...
    if (!is_multiline)
    {
        GetWindowDrawList()->AddRectFilled(frame_bb.Min, frame_bb.Max, GetColorU32(c::input::background), 0.f);
    }

    const ImVec4 clip_rect(frame_bb.Min.x, frame_bb.Min.y, frame_bb.Min.x + inner_size.x, frame_bb.Min.y + inner_size.y); 
    ImVec2 draw_pos = is_multiline ? draw_window->DC.CursorPos : frame_bb.Min + ImVec2(7, 7);
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_selection)
        {
            ...
            ImU32 bg_color = GetColorU32(c::input::text_selected); // FIXME: current code flow mandate that render_cursor is always true here, we are leaving the transparent one for tests.
        ...
        {
            ImU32 col = GetColorU32(is_displaying_hint ? c::input::hint_text : c::input::text);
        ...
        if (render_cursor)
        {
            ...
            it_anim->second = ImLerp(it_anim->second, cursor_screen_rect.Min.x - total_bb.Min.x, g.IO.DeltaTime * 20.f);

            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
                draw_window->DrawList->AddLine(total_bb.Min + ImVec2(it_anim->second, 8), total_bb.GetBL() - ImVec2(-it_anim->second, 8), GetColorU32(ImGuiCol_Text));
...
bool ImGui::ColorEdit3(const char* label, const char* hint_label, float col[3], ImGuiColorEditFlags flags)
{
    return ColorEdit4(label, hint_label, col, flags | ImGuiColorEditFlags_NoAlpha);
}
...
bool ImGui::ColorEdit4(const char* label, const char* hint_label, float col[4], ImGuiColorEditFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel))
    {
        ...
        window->DC.CursorPos.x = pos.x - w_button + ((flags & ImGuiColorEditFlags_NoInputs) ? w_button : w_full + style.ItemInnerSpacing.x);
        GetWindowDrawList()->AddText(window->DC.CursorPos + ImVec2(0, -1), GetColorU32(c::picker::text), label);
        GetWindowDrawList()->AddText(window->DC.CursorPos + ImVec2(0, 13), GetColorU32(c::picker::hint_text), hint_label);
    }
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    const float default_size = GetFrameHeight() + 4.f;
    const float w = GetContentRegionMax().x - GetStyle().WindowPadding.x - default_size;

    ImGui::SetCursorPos(GetCursorPos() - ImVec2(0, 3.f));

    const ImVec2 size(size_arg.x == 0.0f ? default_size : size_arg.x, size_arg.y == 0.0f ? default_size : size_arg.y);
    const ImRect bb(window->DC.CursorPos, window->DC.CursorPos + size + ImVec2(w, 0));
    ItemSize(ImRect(bb.Min, bb.Max - ImVec2(0, 5)));
    ...
    else
    {
        ...
        if (col_source.w < 1.0f)
            RenderColorRectWithAlphaCheckerboard(window->DrawList, bb_inner.Min + ImVec2(w, 0), bb_inner.Max, GetColorU32(col_source), grid_step, ImVec2(off, off), 5.f);
        else
            window->DrawList->AddRectFilled(bb_inner.Min + ImVec2(w, 0), bb_inner.Max, GetColorU32(col_source), 5.f);
    }
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, selected ? c::general_color : c::selectable::line, g.IO.DeltaTime * 6.f);
    it_anim->second.text = ImLerp(it_anim->second.text, selected ? c::general_color : c::selectable::hint_text, g.IO.DeltaTime * 6.f);

    GetWindowDrawList()->AddRectFilled(bb.Min + ImVec2(0, 1), bb.Max - ImVec2(GetContentRegionMax().x, 1), GetColorU32(it_anim->second.background));
    ...
    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text));
    RenderTextClipped(text_min + ImVec2(7, 0), text_max, label, NULL, &label_size, style.SelectableTextAlign, &bb);
    PopStyleColor();
